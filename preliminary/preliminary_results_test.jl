...
Fext_norm = normalizer(Fext_val, minimum(Fext_val), maximum(Fext_val))
...
# First set --> "True" data
sol1 = solve(eqn, Rosenbrock23(); abstol=abstol, reltol=reltol, maxiters=1e7, saveat=t_points)
...
# Second set --> "Modified" data (adjusted damping factor)
...
sol2 = solve(eqn_modified, Rosenbrock23(); abstol=abstol, reltol=reltol, maxiters=1e7, saveat=t_points)
...
# Plot both solutions to visualize difference
p24 = plot(Array(x_time), V1_norm, label = "Original Model", xlabel = "Time (s)", ylabel = "Normalized Voltage")
plot!(p24, Array(x_time), V2_norm, label = "Modified Model")
title!(p24, "Comparison of Model Outputs")
display(p24)

# Plot both solutions to visualize difference (close up)
p25 = plot(Array(x_time[4500:end]), V1_norm[4500:end], label = "Original Model", xlabel = "Time (s)", ylabel = "Normalized Voltage")
plot!(p25, Array(x_time[4500:end]), V2_norm[4500:end], label = "Modified Model")
title!(p25, "Comparison of Model Outputs")
display(p25)
...
for i in 1:6
    plot!(p_traj[i], x_time, original_states[i], label="Original", 
...
# Plot original, modified and UDE predition (close up)
final_pred = predict(res.u)
p27 = plot(x_time[4500:end], y_voltage[4500:end], label = "Original Model", xlabel = "Time (s)", ylabel = "Normalized Voltage")
plot!(p27, x_time[4500:end], V2_norm[4500:end], label = "Modified Model")
plot!(p27, x_time[4500:end], final_pred[6,:][4500:end], label = "UDE Prediction")
title!(p27, "UDE Performance")
display(p27)

# Plot only original model and UDE prediction
...
# Plot only original model and UDE prediction (close up)
p29 = plot(x_time[4500:end], y_voltage[4500:end], label = "Original Voltage", xlabel = "Time (s)", ylabel = "Normalized Voltage")
plot!(p29, x_time[4500:end], final_pred[6,:][4500:end], label = "UDE Prediction")
title!(p29, "Voltage: Original vs UDE Prediction")
display(p29)

# Plot the loss vs epochs (log scale)
p30 = plot(1:length(losses), losses, xlabel = "Epoch", ylabel = "Loss", label = "Training Loss", yscale=:log10)
title!(p30, "Loss vs Epochs")
display(p30)

# Plot the loss vs epochs (linear scale)
p31 = plot(1:length(losses), losses, xlabel = "Epoch", ylabel = "Loss", label = "Training Loss")
title!(p31, "Loss vs Epochs")
display(p31)

# Plot the prediction error
pred_error = final_pred[6,:] .- y_voltage
p32 = plot(x_time, pred_error, xlabel = "Time (s)", ylabel = "Error", label = "Voltage Prediction Error")
title!(p32, "UDE Prediction Error")
display(p32)

# Plot L2 norm error for all states
