...
t_points = 0:dt:0.5  # Match your tspan
...
p_modified.c = 0.0225  # Increase damping by ~33%
...
function ude_dynamics!(du, u, p, t)
    # Get current acceleration and normalize it
    current_acceleration = Fext_input(t)
    ...
    # Normalize the model derivatives using correction_scales
    correction_scales = [
        (x1_max - x1_min),
        (x1dot_max - x1dot_min),
        (x2_max - x2_min),
    ...
    # Neural network works in normalized space
    nn_input = vcat(u, acc_norm)  # Normalized states + normalized acceleration
    nn_correction = U(nn_input, p, _st)[1]  # NN correction in normalized space
...
const U = Lux.Chain(Lux.Dense(7, 32, rbf), # 6 normalized states + 1 normalized acceleration
                    Lux.Dense(32, 32, rbf),
                    #Lux.Dense(32, 64, rbf),
                    #Lux.Dense(64, 64, rbf),
                    #Lux.Dense(64, 32, rbf),
                    Lux.Dense(32, 6) # 6 normalized state corrections
)
...
# Convert parameters to a flat vector for optimization
...
function loss(θ)
    pred = predict(θ)
    if any(isnan, pred)
        println("Warning: NaN detected in prediction")
        return Inf
    end
...
# Modified callback function with intermediate plotting
losses = Float64[]
plot_every = 10  # Plot every 10 iterations

callback = function (θ, l)
    push!(losses, l)
    
    if length(losses) % plot_every == 0
        # Print diagnostics
        println("\nIteration $(length(losses)):")
        println("Loss: $(losses[end])")
        println("Loss change: ", losses[end] - losses[end-1])
        println("Gradient norm: ", norm(θ))
        
        # Get current prediction
        pred = predict(θ)
        
        # Plot comparison
        p = plot(Array(x_time), y_voltage, label="Target (Original)",
                xlabel="Time (s)", ylabel="Normalized Voltage",
                title="Training Progress - Iteration $(length(losses))")
        plot!(p, Array(x_time), V2_norm, label="Modified Model")
        plot!(p, Array(x_time), pred[6,:], label="UDE Prediction")
        
        # Plot loss history
        p_loss = plot(losses, ylabel="Loss", xlabel="Iteration",
                     title="Loss History", yscale=:log10)
        
        # Display plots side by side
        display(plot(p, p_loss, layout=(1,2), size=(1200,400)))
        
        # Check stopping criterion
        if length(losses) > 1 && abs(losses[end] - losses[end-1]) < 1e-8
            println("\nStopping: Loss change too small")
            return true
        end
    end
...
# Start optimization with flattened parameters
optf = OptimizationFunction((θ, p) -> loss(θ), Optimization.AutoZygote())
optprob = OptimizationProblem(optf, p_flat)

res = solve(optprob, OptimizationOptimisers.Adam(), callback = callback, 
            maxiters = 20, progress = true, show_trace = true)  
println("Training loss after $(length(losses)) iterations: $(losses[end])")

# Plot final results
println("\nGenerating final plots...")
final_pred = predict(res.u)
p25 = plot(x_time, y_voltage, label="Original Model", 
           xlabel="Time (s)", ylabel="Normalized Voltage")
...
function visualize_states(x_time, y_true, y_pred, state_names)
    plots = []
    for (i, name) in enumerate(state_names)
        p = plot(x_time, y_true[i,:], 
            label="True", 
            title="$(name) Comparison",
            xlabel="Time",
            ylabel="Normalized $(name)")
        plot!(p, x_time, y_pred[i,:], label="Predicted")
        push!(plots, p)
    end
    return plot(plots..., layout=(3,2), size=(1200,800))
end

state_names = ["x1", "x1dot", "x2", "x2dot", "Qvar", "Voltage"]
p_states = visualize_states(x_time, final_true, final_pred, state_names)

function plot_residuals(x_time, y_true, y_pred)
    residuals = y_true .- y_pred
    p_residual = plot(x_time, residuals[6,:], 
        title="Voltage Residuals", 
        xlabel="Time", 
        ylabel="Residual",
        legend=false)
    
    p_hist = histogram(residuals[6,:], 
        title="Residual Distribution", 
        xlabel="Residual Value", 
        ylabel="Frequency")
    
    return plot(p_residual, p_hist, layout=(1,2))
end

p_residuals = plot_residuals(x_time, y_true, final_pred)

function compute_error_metrics(y_true, y_pred)
...
function plot_prediction_uncertainty(x_time, y_true, y_pred)
    # Compute prediction interval (if multiple runs available)
    lower_bound = y_pred .- std(y_pred, dims=2)
    upper_bound = y_pred .+ std(y_pred, dims=2)
    
    p = plot(x_time, y_true[6,:], 
        label="True Data", 
        title="Prediction with Uncertainty",
        xlabel="Time", 
        ylabel="Normalized Voltage")
    plot!(p, x_time, y_pred[6,:], label="Predicted Mean")
    plot!(p, x_time, lower_bound[6,:], 
        label="Lower Bound", 
        linestyle=:dash)
    plot!(p, x_time, upper_bound[6,:], 
        label="Upper Bound", 
        linestyle=:dash)
