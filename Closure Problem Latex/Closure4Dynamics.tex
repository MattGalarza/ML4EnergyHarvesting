...
\subsection{Classical Approaches to Closure}

\subsubsection{Statistical Closure}
In turbulence modeling, Reynolds averaging leads to the Reynolds-Averaged Navier-Stokes (RANS) equations where the closure problem manifests as the Reynolds stress tensor. Classical approaches include:
\begin{itemize}
    \item \textbf{Eddy viscosity models}: Approximate unresolved stresses through enhanced dissipation
    \item \textbf{Scale similarity models}: Assume self-similar behavior across scales
    \item \textbf{Gradient transport models}: Model fluxes as proportional to mean gradients
\end{itemize}

\subsubsection{Phenomenological Closure}
Many fields develop closure models based on physical reasoning:
\begin{itemize}
    \item \textbf{Subgrid-scale models}: In Large Eddy Simulation (LES), model energy cascade to unresolved scales
    \item \textbf{Coarse-graining}: In molecular dynamics, derive effective potentials for coarse-grained representations
    \item \textbf{Parameterization schemes}: In climate models, represent cloud physics and convection through simplified relationships
\end{itemize}
